...
class HomeFragment : Fragment(), AdapterView.OnItemSelectedListener {
    ...
    private var companyAllNames = ArrayList<String>()
    private var models = HashMap<String, ArrayList<String>>()
    private var modelNames = ArrayList<String>()
    ...
    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
        when (parent?.id) {
            R.id.companySpinner -> {
                companyName = companyAllNames[position]
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        FirebaseDatabase.getInstance().getReference("company").addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                val companyAdapter = ArrayAdapter(activity, android.R.layout.simple_spinner_item, companyAllNames)
                companyAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                companySpinner.adapter = companyAdapter
                companySpinner.onItemSelectedListener = this@HomeFragment

                val modelAdapter = ArrayAdapter(activity,android.R.layout.simple_spinner_item, modelNames)
